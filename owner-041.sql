...
begin
 ...
 if p_operation_type='create' then
  ...
  values(v_id,btrim(p_payload->>'distributor_name'),nullif(btrim(p_payload->>'distributor_reference'),''),p_payload->>'identity_key',nullif(p_payload->>'lead_id','')::uuid,nullif(btrim(p_payload->>'phone'),''),nullif(btrim(p_payload->>'city'),''),(p_payload->>'assigned_to')::uuid,p_payload->>'installation_status',nullif(p_payload->>'installation_completed_at','')::date,p_payload->>'training_status',nullif(p_payload->>'training_completed_at','')::date,coalesce(p_payload->>'mapping_status','pending'),nullif(p_payload->>'mapped_at','')::date,p_payload->>'activity_status',p_payload->>'billing_status',nullif(p_payload->>'billed_at','')::date,nullif(btrim(p_payload->>'bill_reference'),''),nullif(p_payload->>'renewal_date','')::date,p_actor_id)
...
begin
 ...
 v_payload=v_item->'payload';
  ...
  if coalesce(v_payload->>'mapping_status','pending') not in ('pending','done') then return jsonb_build_object('success',false,'code','MAPPING_STATUS_REQUIRED'); end if;
  if coalesce(v_payload->>'mapping_status','pending')='done' and ((v_payload->>'installation_status')<>'done' or (v_payload->>'training_status')<>'done') then return jsonb_build_object('success',false,'code','INVALID_STATUS_COMBINATION'); end if;
 ...
 for v_item in select jsonb_build_object('payload',payload) from distributor_import_stage where classification in ('UPDATE','EXACT_DUPLICATE') order by (payload->>'distributor_id')::uuid loop
  v_payload=v_item->'payload';
  select * into v_current from public.distributor_accounts where distributor_id=(v_payload->>'distributor_id')::uuid for update;
 ...
 for v_item in select jsonb_build_object('classification',classification,'payload',payload) from distributor_import_stage order by row_number loop
  ...
  if v_item->>'classification'='NEW' then
   ...
   update public.distributor_accounts set distributor_name=btrim(v_payload->>'distributor_name'),distributor_reference=nullif(btrim(v_payload->>'distributor_reference'),''),identity_key=v_payload->>'identity_key',assigned_to=(v_payload->>'assigned_to')::uuid,installation_status=v_payload->>'installation_status',installation_completed_at=nullif(v_payload->>'installation_completed_at','')::date,training_status=v_payload->>'training_status',training_completed_at=nullif(v_payload->>'training_completed_at','')::date,mapping_status=case when v_payload ? 'mapping_status' then v_payload->>'mapping_status' else mapping_status end,mapped_at=case when v_payload ? 'mapped_at' then nullif(v_payload->>'mapped_at','')::date else mapped_at end,activity_status=v_payload->>'activity_status',billing_status=v_payload->>'billing_status',billed_at=nullif(v_payload->>'billed_at','')::date,bill_reference=nullif(btrim(v_payload->>'bill_reference'),''),renewal_date=nullif(v_payload->>'renewal_date','')::date,version=version+1,updated_at=now() where distributor_id=(v_payload->>'distributor_id')::uuid returning * into v_current; v_updated=v_updated+1;
