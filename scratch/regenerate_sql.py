...
with open('supabase/migrations/026_team_kpi_repair.sql', 'r', encoding='utf-8') as src:
    migration_content = src.read()

header = '''-- 02_APPLY_TEAM_KPI_026.sql
...
with open('docs/team-kpi-manual-deployment/02_APPLY_TEAM_KPI_026.sql', 'w', encoding='utf-8') as f:
    f.write(header + migration_content + footer)

verify = '''-- 03_VERIFY_TEAM_KPI_026.sql
...
with open('docs/team-kpi-manual-deployment/03_VERIFY_TEAM_KPI_026.sql', 'w', encoding='utf-8') as f:
    f.write(verify)

print("Regenerated 02 and 03 SQL scripts.")
