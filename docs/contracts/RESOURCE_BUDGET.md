...
- Visual intelligence is presentation-only. My Day adds zero requests; Team KPI remains one initial API request. Visits Overview uses one 50-row register request plus one aggregate request with at most 366 buckets per committed filter scope; pagination repeats only the register request. It adds no polling, Realtime channels, Storage reads, or business writes.
