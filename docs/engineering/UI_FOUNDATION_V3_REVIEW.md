...
PR111 run `34249441122` passed registered unit/type/lint proofs but failed the additional related-test gate: `currentErpFootprint.test.ts` still required a duplicate hidden summary and old donut-only copy. This is an `INTERNAL_TASK_DEFECT`, not a kernel or GitHub defect. The correction replaces those implementation-string expectations with rendered exact single-list coverage for six/seven nonzero categories, zero/unknown retention and unavailable-state withholding. The existing test is now mapped and included in `ui-foundation-unit`, so focused UI verification cannot omit it. Run the existing `node scripts/engineering/readiness.mjs --related --base origin/main --head HEAD` gate on the frozen correction head before publication as well. No application or workflow change is required; new exact-head CI remains pending until publication.
