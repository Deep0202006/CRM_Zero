...
[PROOFS.json](PROOFS.json), [INVARIANTS.md](INVARIANTS.md), and the exact
...
The executable loop is Task → Resolve → Implement → Impact → Prove → Certify →
Learn. The semantic graph is generated in Git metadata. Graphify adds optional
navigation evidence only. Production remains behind the Owner gate.
