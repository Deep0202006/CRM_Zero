...
Parity evidence must bind the source manifest, dump/package, and target restore to the same source snapshot. Final cutover separately requires all writers quiesced, final Storage delta certification, and an Owner rollback-availability decision. Because browser Supabase configuration is build-time and CRM queues are durable, a later product proof must require clients onto the target deployment while preserving and exactly-once converging offline work; late managed-source writes require reconciliation. These are G3 blockers, not runtime changes in readiness tooling.
...
The sealed appliance adds proof `supabase-sealed-package`. It produces an Owner-authorized encrypted `SEALED_MIRROR` snapshot only: the migration operator has SFTP upload/read-certificate access, never Docker/root/cloud/source/GitHub/Vercel/credential authority; root-owned target code verifies provenance, nonce, snapshot, and authenticated encryption; and Vercel continues to point at the managed source. A mirror refresh always requires a new Owner authorization.
