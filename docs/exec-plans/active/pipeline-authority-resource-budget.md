...
- Deployed Pipeline triggers create follow-up and registration tasks; broad policies permit Admin mutation; Retailer Payment has 13 rows; legacy cron jobs bypass transition authority and mix a legitimate Distributor renewal transition with prohibited task creation.
...
- Distributor Payment to Renewal Due remains a bounded daily system transition with actorless audit and zero task/follow-up creation.
...
- [ ] Corrected full gates, CI, and regenerated byte-identical owner migration handoff.

## Deferred migration metadata

- Production 037/038 runtime effects were independently certified after manual execution. The blank legacy remote migration-history ledger is deferred to a separate owner-approved maintenance task; `supabase db push` remains prohibited until the complete history is reconciled.

## Blocker correction

- Migration 037 replaces the legacy mixed-purpose renewal function with a bounded Distributor-only stage/audit processor and rebuilds its daily cron while removing task generation.
- Migration 038 explicitly enables the installed stage-write guard for its actorless system correction; the exact Retailer/Payment predicate is rechecked by the update.
- Static and PostgreSQL tests cover ordinary quoted-literal line safety, proven-task cancellation, renewal behavior, stage guard authority, owner/Admin boundaries, and lead intake.
