...
const evidenceLines = evidence.map((item) => `- ${item.status} exit ${item.exitCode ?? "N/A"}: \`${item.command}\``);
const skipped = evidence.filter((item) => item.status === "SKIPPED");
const handoff = Boolean(options.handoff);
const lines = handoff ? [
  "# Implementation handoff", "## Status", evidence.some((item) => item.exitCode !== 0) ? "BLOCKED" : "LOCAL_COMPLETE",
  ...
  "# Detached review pack",
  "<!-- zerodata-harness",
  JSON.stringify({
    taskId: state.taskId,
    area: state.area,
    riskLevel: state.riskLevel,
    databaseChange: state.databaseChangeExpected,
    uiChange: state.uiChangeExpected,
    allowedPaths: boundary.allowedPaths,
    approvedExceptions: [...new Set([...(boundary.explicitlyApprovedAdditionalPaths ?? []), ...(state.explicitlyApprovedAdditionalPaths ?? [])])]
  }, null, 2),
  "-->",
  "## Status", "LOCAL_COMPLETE until required GitHub Actions checks are green.",
  "## Task intent", state.taskDescription,
  ...
  "CI evidence: see the current GitHub Actions run and artifact `zerodata-harness-evidence`.",
