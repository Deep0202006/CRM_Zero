...
if psql -X -Atqc "select 1 from information_schema.columns where table_schema='public' and table_name='mapping_requests' and column_name='requested_by_id_snapshot'" | grep -qx '1'; then
  echo "failed Migration 051 left schema changes behind" >&2
  exit 1
fi
psql -X -v ON_ERROR_STOP=1 -f scripts/mappings-db/bootstrap.sql
