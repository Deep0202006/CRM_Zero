...
do $$
declare a uuid := '10000000-0000-4000-8000-000000000001'; b uuid := '10000000-0000-4000-8000-000000000002'; u uuid := '10000000-0000-4000-8000-000000000004'; r uuid := '20000000-0000-4000-8000-000000000002';
begin
  ...
  begin perform set_config('request.jwt.claim.sub',b::text,true); set local role authenticated; update public.mapping_requests set notes='rewrite' where request_id=r; raise exception 'COMPLETED_REWRITE_ALLOWED'; exception when others then if sqlerrm='COMPLETED_REWRITE_ALLOWED' then raise; end if; end; reset role;
  begin perform set_config('request.jwt.claim.sub',a::text,true); set local role authenticated; delete from public.mapping_requests where request_id=r; raise exception 'DELETE_ALLOWED'; exception when others then if sqlerrm='DELETE_ALLOWED' then raise; end if; end; reset role;
  ...
  if has_function_privilege('authenticated','public.mapping_request_attribution_guard_v1()','execute') or has_function_privilege('anon','public.mapping_request_attribution_guard_v1()','execute') or has_function_privilege('public','public.mapping_request_attribution_guard_v1()','execute') then raise exception 'TRIGGER_EXECUTE_EXPOSED'; end if;
end $$;
