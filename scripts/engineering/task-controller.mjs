...
export const runTaskController = () => {
  ...
  const prepareId = args.value("--prepare"); if (prepareId) { const task = loadTask(prepareId), context = JSON.parse(readFileSync(resolve(taskDirectory(prepareId), "context.json"), "utf8")), writeScope = String(args.value("--write", "")).split(",").map((value) => value.trim().replaceAll("\\", "/")).filter(Boolean); if (!args.value("--outcome") || !args.value("--acceptance") || !args.value("--reproduction") || !args.value("--root-cause") || !writeScope.length) throw new Error("TASK_PREPARE_FIELDS_REQUIRED"); const scoped = writeScope.map((path) => { const absolute = resolve(root, path); return { path, contentHash: existsSync(absolute) ? sha256(readFileSync(absolute)) : "ABSENT", reason: "ROOT_CAUSE_OR_DETERMINISTIC_RELATIONSHIP" }; }); writeTaskArtifact(prepareId, "acceptance.json", { schemaVersion: 1, observableOutcome: args.value("--outcome"), items: args.value("--acceptance").split(";").map((text, index) => ({ id: index + 1, text: text.trim(), status: "PENDING" })).filter((item) => item.text), nonGoals: String(args.value("--non-goals", "")).split(";").filter(Boolean) }); writeTaskArtifact(prepareId, "plan.json", { schemaVersion: 1, reproduction: args.value("--reproduction"), rootCause: args.value("--root-cause"), affectedAuthority: context.authorities ?? [], capabilitiesReused: context.capabilities ?? [], writeScope: scoped, protectedPaths: ["src/**", "public/**", "supabase/**", ".env*", "middleware.*", "proxy.*", "next.config.*", "vercel.json"], risk: context.risk, focusedProof: String(args.value("--proof", "")).split(",").filter(Boolean) }); const next = compareAndSwapTask(prepareId, task.revision, { status: "IMPLEMENTATION_READY" }); appendProgress(prepareId, { event: "INVESTIGATION_PERSISTED", writeScope: scoped.map((item) => item.path), rootCauseHash: sha256(args.value("--root-cause")) }); writeTaskArtifact(prepareId, "handoff.md", `# Task ${prepareId}\n\nStatus: ${next.status}\n\nRoot cause recorded. Exact write scope: ${scoped.map((item) => item.path).join(", ")}\n`); return compact(prepareId); }
