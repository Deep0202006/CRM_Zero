...
import { acknowledgeContextReread, contextRereadPending, readSessionState, sessionsDirectory } from "./hooks/state-store.mjs";
...
export const rereadSessionContext = ({ sessionId } = {}) => {
  const candidates = (sessionId ? [readSessionState(sessionId)] : sessionIds().map((id) => readSessionState(id))).filter(contextRereadPending); if (candidates.length !== 1) throw new Error(`CONTEXT_REREAD_SESSION_AMBIGUOUS:${candidates.length}`);
  const state = acknowledgeContextReread(candidates[0].sessionId); return { schemaVersion: 1, sessionId: state.sessionId, taskId: state.boundTaskId, revision: state.contextAck.revision, sha256: state.contextAck.sha256, status: "ACKNOWLEDGED" };
};

if (process.argv[1] && resolve(process.argv[1]) === resolve(import.meta.filename)) {
  try { const args = parseArgs(), options = { sessionId: args.value("--session") }, result = args.has("--reread") ? rereadSessionContext(options) : args.has("--snapshot") ? sessionSnapshot(options) : sessionStatus(options); console.log(JSON.stringify(result, null, 2)); }
