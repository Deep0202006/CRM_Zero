...
const plan = (base) => JSON.parse(execFileSync("node", ["scripts/engineering/proof-plan.mjs", "--base", base, "--head", "HEAD"], { cwd: root, encoding: "utf8" }));
...
try {
  const pr = JSON.parse(run(["pr", "view", "--json", "number,headRefOid,baseRefOid"]));
  ...
  if (execFileSync("git", ["merge-base", "HEAD", pr.baseRefOid], { cwd: root, encoding: "utf8" }).trim() !== pr.baseRefOid) output("REMOTE_EVIDENCE_REQUIRED", { code: "PR_BASE_NOT_CONTAINED", baseRefOid: pr.baseRefOid }, 2);
  const proof = plan(pr.baseRefOid);
  ...
  if (/no checks reported/i.test(String(error))) output("REMOTE_EVIDENCE_PENDING", { remoteEvidence: { status: "PENDING", headSha: repo.currentHeadSha, treeSha: repo.currentTreeSha, planHash: plan("origin/main").planHash, requiredChecks: [], terminalChecks: [] } }, 2);
