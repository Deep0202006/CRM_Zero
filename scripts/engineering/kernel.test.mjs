...
try {
  ...
  const stableRpcPatch = "diff --git a/src/lib/distributors/reader.ts b/src/lib/distributors/reader.ts\n+++ b/src/lib/distributors/reader.ts\n+await supabase.rpc('fixture_projection_v1', {});\ndiff --git a/supabase/migrations/900_fixture.sql b/supabase/migrations/900_fixture.sql\n+++ b/supabase/migrations/900_fixture.sql\n+create function public.fixture_projection_v1() returns jsonb language sql stable as $$ select '{}'::jsonb $$;";
  const stableRpcRead = compileImpact({ entries: [{ status: "M", path: "src/lib/distributors/reader.ts" }, { status: "A", path: "supabase/migrations/900_fixture.sql" }], patch: stableRpcPatch, selectedDomains: ["distributor-status"] });
  assert.equal(stableRpcRead.writable, true); assert.equal(stableRpcRead.writeOperations.length, 0); assert.equal(stableRpcRead.readOperations[0].functionName, "public.fixture_projection_v1");
  matrix.risk.push("nul-rename-delete", "sensitive-unmapped-r3", "multiline-fact-write-detected", "unknown-authority", "prohibited-authority", "immutable-migration-001-051", "pr90-reproduction", "new-column-selector-required", "domain-local-prohibition", "renewal-owned-migration", "financial-write-isolation", "financial-read-only", "sql-function-enclosure", "authority-ambiguity", "structured-supabase-writes", "stable-rpc-read-only");
