...
import { invalidatePrepushCertificate, isGitStateChangingCommand, isGitStateRevalidationCommand, recordFailure, updateTaskMetrics } from "../experience.mjs";
...
if (activeTask && !exitCode && /\bgit(?:\.exe)?\s+push\s+origin\s+(?:feat|fix|chore)\//i.test(shellCommand)) updateTaskMetrics(activeTask.taskId, (metrics) => ({ ...metrics, pushCount: (metrics.pushCount ?? 0) + 1 }));
