...
export const doctor = ({ ci = false } = {}) => {
  ...
  const boundary = JSON.parse(readFileSync(resolve(root, "supabase/migrations/APPLIED_OWNER_MIGRATIONS.json"), "utf8"));
  if (boundary.lastAppliedOwnerMigration !== 51 || boundary.immutableThrough !== 51) failures.push("MIGRATION_BOUNDARY");
  return { status: failures.length ? "SAFETY_CONFLICT" : "KERNEL_HEALTHY", repository: "Deep0202006/CRM_Zero", headSha: git("rev-parse", "HEAD"), treeSha: git("rev-parse", "HEAD^{tree}"), branch: git("branch", "--show-current"), migrationBoundary: `${boundary.lastAppliedOwnerMigration}/${boundary.immutableThrough}`, failures };
};
