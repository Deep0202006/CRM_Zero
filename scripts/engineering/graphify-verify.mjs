...
const stampPath = resolve(root, "graphify-out/.crm-tree");
...
if (!existsSync(stampPath) || readFileSync(stampPath, "utf8").trim() !== execFileSync("git", ["rev-parse", "HEAD^{tree}"], { cwd: root, encoding: "utf8" }).trim()) throw new Error("GRAPHIFY_STALE");
