...
export default function OnboardingPage() {
  ...
  return <div className="app-page relative">
    ...
    <section className="-mx-4 overflow-x-auto px-4 pb-3 sm:-mx-6 sm:px-6 lg:-mx-8 lg:px-8" aria-label={`${segmentTab} pipeline board`}><div className="flex min-w-max gap-3">
      {PIPELINE_STAGES.map((stage) => { const stageLeads = visibleLeads.filter((lead) => lead.status === stage); return <article key={stage} className="flex w-[292px] shrink-0 flex-col rounded-[var(--radius-lg)] border border-[var(--border-subtle)] bg-[var(--surface-secondary)]">
        <header className="flex items-center justify-between border-b border-[var(--border-subtle)] px-3.5 py-3"><div><p className="text-[12px] font-semibold">{stage}</p><p className="text-[10px] text-[var(--text-muted)]">{stageLeads.length} in this stage</p></div><Chip variant={STAGE_VARIANTS[stage]} size="sm">{stageLeads.length}</Chip></header>
        <div className="min-h-[220px] space-y-2 p-2.5">{stageLeads.map((lead) => { const state = pending.get(lead.lead_id); const actions = getEmployeeTransitionActions(lead.status); const isOwner = lead.assigned_to === currentUser?.user_id; return <div key={lead.lead_id} className="rounded-[var(--radius-md)] border border-[var(--border-subtle)] bg-[var(--surface-primary)] p-3 shadow-[var(--shadow-raised)]">
