...
export async function GET(request: Request, { params }: { params: Promise<{ leadId: string }> }) {
  ...
  const [ownerResult, tasksResult, callsResult, transitionsResult] = await Promise.all([
    ...
    context.service.from("tasks").select("task_id,title,priority,status,due_date,completed_at,created_at,assigned_to").eq("related_lead_id", leadId).eq("is_active", true).order("due_date", { ascending: true }).order("task_id", { ascending: true }).limit(20),
    context.service.from("call_logs").select("log_id,user_id,timestamp,outcome,notes,next_followup_date").eq("lead_id", leadId).order("timestamp", { ascending: false }).order("log_id", { ascending: false }).limit(10),
  ...
  return Response.json({
    ...
    latest_call: callsResult.data?.[0] ?? null,
