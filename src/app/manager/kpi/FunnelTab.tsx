...
export default function FunnelTab() {
  ...
  useEffect(() => {
    ...
    const deadline = setTimeout(() => { setError("Pipeline inspection took too long. Retry the applied scope."); setLoading(false); controller.abort(); }, 12000);
    void (async () => {
  ...
  return <div className="space-y-4" aria-busy={loading}>
    ...
    {inspection ? <>
      <p className="text-xs text-[var(--text-secondary)]">Applied: {appliedSegment} · Refreshed {new Date(inspection.scope.generated_at).toLocaleString("en-IN", { timeZone: "Asia/Kolkata" })} IST. Stage counts use the same search, owner and source, across all stages and attention states. Event history, stage age and source conversion remain segment-wide.</p>
      {!!inspection.analytics_unavailable?.length && <p role="status" className="alert-panel alert-panel--warning">Unavailable: {inspection.analytics_unavailable.join(", ")}. Available records remain usable; missing metrics are not zero.</p>}
      ...
      <Button variant="secondary" type="submit">Search</Button>
            ...
            <details><summary className="cursor-pointer text-sm">Register filters</summary><div className="grid gap-3 sm:grid-cols-3"><label className="text-sm">Stage<select aria-label="Stage" className="field-control" value={stage} onChange={(event) => setStage(event.target.value)}><option value="">All stages</option>{stages.map((row) => <option key={row.stage} value={row.stage}>{row.stage}</option>)}</select></label>      <select className="field-control min-w-0" aria-label="Owner" value={owner} onChange={(event) => setOwner(event.target.value)}><option value="">All owners (up to 50 active choices below)</option>{inspection.owner_options.map((item) => <option key={item.user_id} value={item.user_id}>{item.name}</option>)}</select>
      ...
      <div className="grid gap-4 xl:grid-cols-2">
        <section className="workspace-register p-4"><h2 className="text-base font-semibold">Completed stage intervals</h2>{inspection.analytics_unavailable?.includes("Event history") ? <p>Completed intervals unavailable: retained events could not be read.</p> : <><p className="text-xs text-[var(--text-secondary)]">P50 and mean duration in days · n={inspection.historical_velocity.sample_n} · {inspection.historical_velocity.coverage_n} candidate pairs · {inspection.historical_velocity.coverage_pct}% covered</p><div className="overflow-auto" tabIndex={0} role="region" aria-label="Completed interval duration"><table className="w-full"><thead><tr><th>Stage</th><th>P50 days</th><th>Mean days</th><th>Sample n</th></tr></thead><tbody>{velocity.map((row) => <tr key={row.stage}><th scope="row">{row.stage}</th><td>{row.p50_days}</td><td>{row.average_days}</td><td>{row.sample_n}</td></tr>)}</tbody></table></div>{!velocity.length && <p>No completed intervals in the retained sample.</p>}</>}<details><summary className="cursor-pointer text-sm">Current stage age · not duration</summary>{inspection.analytics_unavailable?.includes("Current stage age") ? <p>Current stage age unavailable.</p> : <dl>{currentAge.map((row) => <div key={row.stage} className="flex justify-between gap-3 text-sm"><dt>{row.stage}</dt><dd>{row.average_days} mean days</dd></div>)}</dl>}</details></section>
        <section className="workspace-register p-4"><h2 className="text-base font-semibold">Source conversion</h2><p className="text-xs text-[var(--text-secondary)]">Reconciled segment-wide source counts · Not filtered by inspection-list controls</p><div className="overflow-auto" tabIndex={0} role="region" aria-label="Source conversion data"><table className="w-full"><thead><tr><th>Source</th><th>Total</th><th>Converted</th><th>Rate</th></tr></thead><tbody>{sources.map((row) => <tr key={row.source}><th scope="row">{row.source}</th><td>{row.total.toLocaleString("en-IN")}</td><td>{row.converted.toLocaleString("en-IN")}</td><td>{row.total > 0 ? row.rate + "%" : "Unavailable"}</td></tr>)}</tbody></table></div>{!sources.length && <p>{inspection.analytics_unavailable?.includes("Source conversion") ? "Source conversion unavailable: failed or more than 100 source groups." : "No source records in this segment."}</p>}</section>
      </div>
