...
const filters: ReadonlyArray<{ label: string; value: RenewalFilter }> = [
  ...
  { label: "In 2 Days", value: "in_two_days" },
...
export function PaymentRenewalsPage({ admin }: { admin: boolean }) {
  ...
  const [erps, setErps] = useState<Array<{ erp_id: string; erp_name: string }>>([]);
  ...
  const listUrl = useMemo(
    () =>
      `/api/distributors/renewals?view=list&filter=${filter}&page=${page}&pageSize=50${erpFilter === "__unset" ? "&erpUnset=true" : erpFilter ? `&erp=${erpFilter}` : ""}`,
    [filter, page, erpFilter],
  );
  ...
  const readJson = useCallback(async (response: Response) => {
    ...
    if (!response.ok)
      throw new Error(
        response.status === 403
          ? "You are not authorized to view these renewals."
          : (result.message ?? "Unable to load renewals."),
      );
    return result;
  }, []);
  ...
  const loadMetrics = useCallback(async () => {
    const result = await readJson(
      await authFetch("/api/distributors/renewals?view=metrics"),
    );
    setMetrics(result.metrics);
  }, [authFetch, readJson]);
  useEffect(() => {
    if (!admin || !isAdmin) return;
    void authFetch("/api/erp-systems").then(readJson).then((result) => setErps(result.rows ?? [])).catch(() => setErps([]));
  }, [admin, authFetch, isAdmin, readJson]);
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      setMetricsError("");
      void loadMetrics().catch((cause) => {
        if (active)
          setMetricsError(
            cause instanceof Error
              ? cause.message
              : "Unable to load renewal metrics.",
          );
      });
    }, 0);
    return () => {
  ...
  return (
    <div className="app-page">
      <PageHeader
        ...
        title="Renewals"
        description="Operational renewal dates from the canonical distributor authority."
      />
      {message && (
      ...
      {metricsError && (
        <div className="alert-panel alert-panel--danger" role="alert">
          Unable to load renewal metrics. {metricsError}
        </div>
      )}
      {listError && (
        <div className="alert-panel alert-panel--danger" role="alert">
          Unable to load renewals. {listError}
        </div>
      )}
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        {cards.map((card) => (
          <button
            ...
            className={`surface-panel p-4 text-left ${filter === card.filter ? "ring-2 ring-[var(--brand-500)]" : ""}`}
            onClick={() => {
        ...
        <div className="flex flex-wrap items-center gap-3 p-4">
          <label className="text-xs font-semibold">
            ...
            <select
              ...
              className="field-control ml-2"
              value={filter}
              onChange={(event) => {
                setFilter(event.target.value as RenewalFilter);
                setPage(1);
              }}
            >
              {filters.map((option) => (
          ...
          {admin && <label className="text-xs font-semibold">ERP<select aria-label="ERP" className="field-control ml-2" value={erpFilter} onChange={(event) => { setErpFilter(event.target.value); setPage(1); }}><option value="">All ERP systems</option><option value="__unset">ERP Not Set</option>{erps.map((erp) => <option key={erp.erp_id} value={erp.erp_id}>{erp.erp_name}</option>)}</select></label>}
        </div>
        {loading ? (
          <p className="p-6 text-sm text-[var(--text-muted)]">
            Loading renewals…
          </p>
        ) : !listError && !metricsError && rows.length === 0 ? (
          <p className="p-6 text-sm text-[var(--text-muted)]">
            {filter === "all"
              ? "No renewal dates set yet."
                  ...
                  {rows.map((row) => (
                    <tr className="border-t" key={row.distributor_id}>
                      ...
                      <td className="p-3">
                        <Button
                          variant="outline"
                          onClick={() => setEditingId(row.distributor_id)}
                          aria-label={`Set renewal for ${row.distributor_name}`}
                        >
                          Set Renewal
                        </Button>
                      </td>
      ...
      <RenewalEditorModal
        ...
        onSave={() => {
          setEditingId(null);
          setMessage("Renewal saved successfully.");
          setLoading(true);
          void loadMetrics().catch((cause) =>
            setMetricsError(
              cause instanceof Error
