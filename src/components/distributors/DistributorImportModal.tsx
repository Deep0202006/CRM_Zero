...
export function DistributorImportModal({ open, authFetch, onClose, onImported }: { open: boolean; authFetch: (url: string, init?: RequestInit) => Promise<Response>; onClose: () => void; onImported: () => Promise<void> }) {
  ...
  function template() {
    void import("xlsx").then((XLSX) => {
      ...
      const sheet = XLSX.utils.aoa_to_sheet([[...DISTRIBUTOR_IMPORT_HEADERS], ["Example Distributor", "employee@example.com", "pending", "", "pending", "", "pending", "", "not_applicable", "not_billed", "", "", "2026-12-31", "DIST-001", "MARG"]]);
  ...
  const blocked = invalid.length > 0 || Boolean((preview?.counts.AMBIGUOUS ?? 0) + (preview?.counts.INVALID_EMPLOYEE ?? 0) + (preview?.counts.ERP_REQUIRED ?? 0));
