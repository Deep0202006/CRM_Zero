...
type Segment = { unique_businesses: number; coverage_percent: number; categories: Array<{ erp_name: string; count: number; share_percent: number }> };
export default function FieldVisitErpIntelligence({ segments }: { segments: Record<string, Segment> }) {
  const retailer = segments.Retailer, distributor = segments.Distributor;
  const panel = (name: string, value?: Segment) => <AnalyticsPanel eyebrow="Current footprint" title={`${name} ERP Footprint`} description="Latest confirmed ERP observation per unique business. Each business is counted once; repeat visits do not inflate totals." labelledBy={`${name}-erp-footprint`}>
    {!value?.unique_businesses ? <AnalyticsEmptyState title="No field observations" description="ERP observations will appear after confirmed visits." /> : <div className="space-y-3">{value.categories.map((category) => <div key={category.erp_name}><div className="mb-1 flex justify-between gap-3 text-xs"><span>{category.erp_name}</span><span>{category.count} · {category.share_percent}%</span></div><div className="h-2 overflow-hidden rounded bg-[var(--surface-secondary)]"><div className="h-full rounded bg-[var(--viz-1)]" style={{ width: `${category.share_percent}%` }} /></div></div>)}</div>}
  </AnalyticsPanel>;
