...
export function ErpDistributionDonut({ title, description, total, totalLabel, categories, reconciled, emptyTitle, emptyDescription, labelledBy, ariaLabel }: {
  ...
  return <AnalyticsPanel eyebrow="Current footprint" title={title} description={description} labelledBy={labelledBy}>
    {!total ? <AnalyticsEmptyState title={emptyTitle} description={emptyDescription} /> : !reconciled ? <AnalyticsEmptyState title="ERP footprint unavailable" description="Category totals do not reconcile with the displayed total, so the chart is hidden instead of presenting misleading intelligence." /> : visibleSlices.length > 6 ? <ChartContainer config={{ count: { label: totalLabel, color: "var(--viz-primary)" } }} className="h-[320px] w-full" >
