...
export function ErpDistributionDonut({ title, description, total, totalLabel, categories, reconciled, emptyTitle, emptyDescription, labelledBy, ariaLabel }: {
  ...
  return <AnalyticsPanel eyebrow="Current footprint" title={title} description={description} labelledBy={labelledBy}>
    {!total ? <AnalyticsEmptyState title={emptyTitle} description={emptyDescription} /> : !reconciled ? <AnalyticsEmptyState title="ERP footprint unavailable" description="Category totals do not reconcile with the Distributor total, so the donut is hidden instead of presenting misleading intelligence." /> : <div className="grid gap-4 lg:grid-cols-[minmax(220px,1fr)_minmax(220px,1fr)]" aria-label={ariaLabel ?? `${title}: ${total} ${totalLabel.toLowerCase()}`}>
