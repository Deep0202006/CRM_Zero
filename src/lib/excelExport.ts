...
export async function exportMappings(userId: string) {
  ...
  const data = mappings.map(m => ({
    'Logged By': users.get(m.requested_by ?? m.requested_by_id_snapshot ?? '') || m.requested_by_name_snapshot || 'Unknown/Former employee',
    'Completed By': users.get(m.mapped_by ?? m.mapped_by_id_snapshot ?? '') || m.mapped_by_name_snapshot || 'Unknown/Former employee',
...
export async function exportMasterMappings() {
  ...
  const data = requests.map(r => ({
    ...
    'Logged By': userMap.get(r.requested_by ?? r.requested_by_id_snapshot ?? '')?.trim() || r.requested_by_name_snapshot || 'Unknown/Former employee',
    'Completed By': userMap.get(r.mapped_by ?? r.mapped_by_id_snapshot ?? '')?.trim() || r.mapped_by_name_snapshot || '',
    'Logged By User ID': r.requested_by ?? r.requested_by_id_snapshot ?? '',
    'Completed By User ID': r.mapped_by ?? r.mapped_by_id_snapshot ?? '',
    'Status': r.status,
    'Notes': r.notes || '',
