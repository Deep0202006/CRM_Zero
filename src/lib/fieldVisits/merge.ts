  ...
  for (const visit of remoteVisits) {
    if (visit.user_id === userId) {
      ...
      merged.set(visit.visit_id, {
        ...visit,
        sync_status: "synced",
        sync_stage: local?.sync_stage === "visit_confirmed_evidence_pending" || local?.sync_stage === "visit_confirmed_link_pending" || local?.sync_stage === "review_required" ? local.sync_stage : "synced",
