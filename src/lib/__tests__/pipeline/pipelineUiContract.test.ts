...
describe("Pipeline UI contract", () => {
  ...
  test("eight readable columns and their lead lists own separate axes", () => {
  ...
  test("cards contain long content and later Payment/Renewal columns remain canonical", () => {
    expect(page).toContain("min-w-0 overflow-hidden"); expect(page).toContain("PIPELINE_STAGES.map");
  });
});
