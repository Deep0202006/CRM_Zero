...
describe("final field visit recovery compatibility", () => {
  ...
  it("uses an exact core whitelist and keeps hardened fields optional", () => {
    ...
    expect(Object.keys(coreRemotePayload(parsed, parsed.attendance_id!))).toEqual([
      "visit_id", "lead_id", "user_id", "visit_date", "check_in_time", "check_in_lat", "check_in_lng", "check_in_photo_url", "visit_outcome", "visit_notes", "attendance_id", "person_met", "segment_type", "follow_up_date", "created_at", "updated_at",
    ]);
