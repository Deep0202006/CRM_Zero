...
describe("receivables exact money and alert contract", () => {
  test.each([
    ["1000", BigInt(100000)], ["84,500", BigInt(8450000)], ["₹84,500.00", BigInt(8450000)], ["0.01", BigInt(1)],
  ])("parses %s exactly", (value, expected) => expect(parseMoneyToMinorUnits(value)).toBe(expected));
  test.each(["0", "-1", "1.001", "nope", "1000000000000.00"])("rejects %s", value => expect(() => parseMoneyToMinorUnits(value)).toThrow());
