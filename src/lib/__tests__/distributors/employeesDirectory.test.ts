...
function service(
  ...
  adminIds: string[] = [],
  errors: { users?: unknown; capabilities?: unknown; auth?: unknown } = {},
) {
  return {
    auth: {
      admin: {
    ...
    from(table: string) {
      const result =
        table === "users"
          ...
          : {
              data: adminIds.map((user_id) => ({ user_id })),
              error: errors.capabilities ?? null,
            };
      const builder = {
        select: () => builder,
        eq: () => builder,
...
describe("canonical operational employee directory", () => {
  test("resolves Auth login email to Vaibhav's active public UUID when public email differs", async () => {
    const vaibhav = {
      user_id: "f2750ad8-f480-4d1d-b8a0-e00190534855",
      name: "Vaibhav Patel",
      email: "zerodata_vaibhav",
      is_active: true,
    };
    const result = await listEligibleOperationalEmployees(
      service(
        [vaibhav],
        [{ id: vaibhav.user_id, email: "zerodata_vaibhav@zerodata.local" }],
      ),
    );
    expect(result).toEqual({
      employees: [
        {
          user_id: vaibhav.user_id,
          name: "Vaibhav Patel",
          email: "zerodata_vaibhav@zerodata.local",
        },
      ],
      error: null,
    });
  ...
  test("excludes inactive profiles", async () => {
    expect(
      await listEligibleOperationalEmployees(
        service(
          [
            {
              user_id: "inactive",
              name: "Inactive",
              email: "inactive-alias",
              is_active: false,
            },
          ],
          [{ id: "inactive", email: "inactive@zerodata.local" }],
        ),
      ),
    ).toEqual({ employees: [], error: null });
  });
    ...
    expect(
      await listEligibleOperationalEmployees(
        service(
          ...
          [{ id: "admin", email: "admin@zerodata.local" }],
          ["admin"],
        ),
      ),
    ).toEqual({ employees: [], error: null });
  ...
  test("excludes ERP Partner Viewer accounts from operational assignments", async () => {
    expect(
      await listEligibleOperationalEmployees(
        service(
          [
          ...
          [{ id: "erp-partner", email: "partner@example.com" }],
          ["erp-partner"],
        ),
      ),
    ).toEqual({ employees: [], error: null });
  ...
  test("rejects an unknown Auth login identity", async () => {
    ...
    expect(
      await listEligibleOperationalEmployees(service([profile], [])),
    ).toEqual({ employees: [], error: null });
  });
    ...
    expect(
      await listEligibleOperationalEmployees(
        service(
          ...
          [
            { id: "employee", email: "duplicate@zerodata.local" },
            { id: "other", email: "duplicate@zerodata.local" },
          ],
        ),
      ),
    ).toEqual({ employees: [], error: null });
  ...
  test("fails closed when either authority query fails", async () => {
    ...
    expect(
      (
        await listEligibleOperationalEmployees(
          service([], [], [], { capabilities: new Error("caps") }),
        )
      ).employees,
    ).toEqual([]);
    expect(
  ...
  test("accepts 500 rows and rejects an over-limit response", async () => {
    ...
    const authUsers = rows.slice(0, 500).map((row) => ({
      id: row.user_id,
      email: `${row.user_id}@zerodata.local`,
    }));
    expect(
      (
        await listEligibleOperationalEmployees(
          service(rows.slice(0, 500), authUsers),
        )
      ).employees,
    ).toHaveLength(500);
    const overflow = await listEligibleOperationalEmployees(
      service(rows, authUsers),
    );
    expect(overflow.employees).toEqual([]);
  ...
  test("Distributor import preview assigns Vaibhav from the Auth login email", async () => {
    ...
    const preview = await buildDistributorImportPreview(
      service(
        ...
        [{ id, email: "zerodata_vaibhav@zerodata.local" }],
      ),
