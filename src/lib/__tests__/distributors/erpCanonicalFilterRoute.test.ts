...
describe("Distributor canonical ERP filter route", () => {
  ...
  it("passes every status-card filter to the bounded server projection", async () => {
    const response = await GET(new Request("http://localhost/api/distributors?installation=done&training=pending&mapping=done&activity=active&renewal=due_soon"));
    expect(response.status).toBe(200);
