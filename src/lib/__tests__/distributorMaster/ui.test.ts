...
const modalPath = path.join(
  process.cwd(),
  "src/components/distributors/DistributorMasterImportModal.tsx",
);
const pagePath = path.join(
  process.cwd(),
  "src/app/admin/payments/distributors/page.tsx",
);
...
describe("distributor master import admin UI", () => {
  ...
  test("previews every sheet with the required summary and Before to After table", () => {
    ...
    expect(modal).toContain(
      '["Sheet", "Row", "Distributor", "ERP", "Bill", "Current State", "Action", "Result State", "Reason"]',
    );
    for (const label of [
      "New Distributors:",
      "Distributor Updates:",
  ...
  test("uses preview-bound safe confirmation and retains a completion summary", () => {
    ...
    expect(modal).toContain(
      'form.set("resolved_plan_hash", preview.resolvedPlanHash)',
    );
