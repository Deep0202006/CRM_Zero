...
const distributor: MasterReceivableDistributor = {
  distributorId: "40000000-0000-4000-a000-000000000001",
  distributorName: "Alpha",
  distributorReference: "ALPHA-1",
  billingStatus: "billed",
  assignedTo: employee.user_id,
};
const row: MasterReceivableRow = {
  rowNumber: 2,
  distributorReference: "ALPHA-1",
  billReference: "INV-1",
  ...
  notes: "Initial obligation",
};

function authority(
  overrides: Partial<MasterReceivableAuthority> = {},
...
describe("master Receivables planning", () => {
  test("creates one canonical obligation with row-independent stable UUID", () => {
    const first = planMasterReceivableRows(
      operationId,
      [row],
      [employee],
      [distributor],
    ...
    const moved = planMasterReceivableRows(
      operationId,
      [{ ...row, rowNumber: 99 }],
      [employee],
      [distributor],
    ...
    expect(masterReceivableMutationRows([first])[0]).toMatchObject({
      row_number: 2,
      receivable_id: first.resolvedReceivableId,
    });
  });
  ...
  test("uses the stable UUID of a Distributor created earlier in the same workbook plan", () => {
    const distributorRow = {
      ...
      installationStatus: "done" as const,
      installationDate: "2026-08-10",
      trainingStatus: "done" as const,
    ...
    const distributorPlan = planMasterDistributorRows(
      operationId,
      [distributorRow],
      [employee],
      [],
    ...
    const receivable = planMasterReceivableRows(
      operationId,
      [row],
      [employee],
      resolved,
    ...
    expect(receivable).toMatchObject({
      classification: "NEW",
      payload: { distributor_id: distributorPlan[0].payload?.distributor_id },
    });
  });

  test("skips an exact canonical bill identity even when display reference casing differs", () => {
    const planned = planMasterReceivableRows(
      operationId,
      [{ ...row, billReference: " inv-1 " }],
      [employee],
      [distributor],
      [authority()],
    )[0];
    expect(planned).toMatchObject({
      ...
      resolvedReceivableId: authority().receivable_id,
      before: { receivable_id: authority().receivable_id },
    });
    expect(masterReceivableMutationRows([planned])).toEqual([
      expect.objectContaining({
        row_number: 2,
        receivable_id: authority().receivable_id,
      }),
    ]);
  });
  ...
  test("fails closed for unresolved or unbilled distributors, invalid employees, and stale follow-up", () => {
    expect(
      planMasterReceivableRows(operationId, [row], [employee], [], [])[0]
        .classification,
    ).toBe("INVALID_DISTRIBUTOR");
    expect(
      planMasterReceivableRows(
        operationId,
        [row],
        [employee],
        [{ ...distributor, billingStatus: "not_billed" }],
        [],
      )[0].classification,
    ).toBe("INVALID_DISTRIBUTOR_STATUS");
    expect(
      planMasterReceivableRows(operationId, [row], [], [distributor], [])[0]
        .classification,
  ...
  test("treats independent canonical contact, assignee, dates, and money as duplicate-critical", () => {
    for (const changed of [
      { contact_phone: "998" },
      { assigned_to: "20000000-0000-4000-a000-000000000002" },
      { bill_due_date: "2026-08-02" },
  ...
  test("derives assignment from the post-import Distributor instead of a Receivables column", () => {
    const reassigned = {
      ...distributor,
      assignedTo: "20000000-0000-4000-a000-000000000002",
    };
    const nextEmployee = {
      ...employee,
      user_id: reassigned.assignedTo,
      name: "Next Employee",
    ...
    expect(planned).toMatchObject({
      classification: "NEW",
      payload: { assigned_to: reassigned.assignedTo },
      assignedEmployeeName: nextEmployee.name,
    });
  });

  test("requires follow-up only when the complete planned payment result remains outstanding", () => {
    const noFollowUp = planMasterReceivableRows(
      operationId,
    ...
    const paid = finalizeMasterReceivableFollowUps(
      [noFollowUp],
      [
        {
          resolvedReceivableId: noFollowUp.resolvedReceivableId,
          classification: "NEW",
          after: { outstandingAmount: "0.00" },
        } as never,
      ],
      "2026-08-20",
    )[0];
    expect(paid.classification).toBe("NEW");
    const outstanding = finalizeMasterReceivableFollowUps(
      [noFollowUp],
      [
        {
          resolvedReceivableId: noFollowUp.resolvedReceivableId,
          classification: "NEW",
          after: { outstandingAmount: "0.01" },
        } as never,
      ],
      "2026-08-20",
    )[0];
    expect(outstanding).toMatchObject({
      classification: "INVALID_FOLLOW_UP_DATE",
      payload: null,
    });
  });
  ...
  test("migration resolver is service-only, exact-keyed, bounded, and read-only", () => {
    ...
    expect(sql).toContain(
      "r.distributor_id=i.distributor_id and r.bill_reference_key=i.bill_reference_key",
    );
    ...
    const start = sql.indexOf(
      "create or replace function public.resolve_distributor_master_receivables_v1",
    );
    const resolver = sql.slice(
      start,
      sql.indexOf(
        "revoke all on function public.resolve_distributor_master_receivables_v1",
        start,
      ),
    );
