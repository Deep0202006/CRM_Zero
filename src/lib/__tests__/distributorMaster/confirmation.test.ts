...
import {
  confirmMasterImport,
  masterConfirmationRequestHash,
  MasterConfirmationError,
} from "@/lib/distributorMaster/confirmation";
import type { MasterImportPreview } from "@/lib/distributorMaster/preview";

const preview = {
  format: "CRM_DISTRIBUTOR_MASTER_V1",
  operationId: "30000000-0000-4000-a000-000000000001",
  businessDate: "2026-08-20",
  rows: { distributors: [], receivables: [], payments: [] },
  counts: {
    distributors: {},
    receivables: {},
    payments: {},
  ...
  resolvedPlanHash: "a".repeat(64),
  execution: {
    distributors: [
      { rowNumber: 2, classification: "NEW", payload: { distributor_id: "d" } },
    ],
    receivables: [{ row_number: 2, receivable_id: "r" }],
    payments: [{ row_number: 2, payment_id: "p" }],
  },
...
describe("atomic master confirmation", () => {
  test("sends the complete revalidated execution plan through exactly one mutation RPC", async () => {
    const rpc = jest
      ...
      .mockResolvedValue({
        data: {
          success: true,
          batch_id: "70000000-0000-4000-a000-000000000001",
          replayed: false,
        },
        error: null,
      });
  ...
  test("request hash binds filename and every domain execution payload", () => {
    ...
    expect(masterConfirmationRequestHash(preview, "other.xlsx")).not.toBe(
      original,
    );
    expect(
      masterConfirmationRequestHash(
        { ...preview, execution: { ...preview.execution, payments: [] } },
        "master.xlsx",
      ),
    ).not.toBe(original);
  });
  ...
  test("typed rejection is terminal while unreadable RPC failure remains uncertain", async () => {
      ...
      confirmMasterImport(
        {
          rpc: jest
            .fn()
            .mockResolvedValue({
              data: { success: false, code: "DISTRIBUTOR_CONFLICT" },
              error: null,
            }),
        } as never,
        "10000000-0000-4000-a000-000000000001",
    ...
    ).rejects.toMatchObject({ code: "DISTRIBUTOR_CONFLICT", uncertain: false });
    await expect(
      confirmMasterImport(
        {
          rpc: jest
      ...
      code: "MASTER_CONFIRMATION_UNCERTAIN",
      uncertain: true,
    });
    await expect(
      confirmMasterImport({ rpc: jest.fn() } as never, "actor", "master.xlsx", {
        ...preview,
        blocking: true,
      }),
    ).rejects.toBeInstanceOf(MasterConfirmationError);
  });
  ...
  test("Migration 046 uses one outer subtransaction and rolls back deterministic child failure", () => {
    ...
    const end = sql.indexOf(
      "revoke all on function public.import_distributor_master_v1",
      start,
    );
    ...
    expect(fn).toMatch(
      /exception\s+when sqlstate 'ZD101' or sqlstate 'ZD104' or sqlstate 'ZD106'/,
    );
    expect(
      fn.indexOf("insert into public.distributor_master_import_batches"),
    ).toBeLessThan(fn.search(/exception\r?\n\s+when sqlstate/));
    expect(sql).toContain("enable row level security");
    expect(sql).toContain(
      "grant execute on function public.import_distributor_master_v1",
    );
  });
});
