...
test("Funnel consumer preserves fractional days, percentages and grouped counts after lazy reveal", async ({ page }) => {
  ...
  const funnel = page.getByRole("tab", { name: /Pipeline funnel/ });
  await expect(funnel).toBeVisible();
  expect(requests).toBe(0);
  await funnel.click();
  const age = panel(page, "Average current age by stage");
  await expect(age).toBeVisible();
  await expect(await hoverCategory(page, age, "New")).toContainText("2.5 days");
  const velocity = panel(page, "Historical stage velocity");
  const velocityTooltip = await hoverCategory(page, velocity, "New");
  await expect(velocityTooltip).toContainText("P50 days");
  await expect(velocityTooltip).toContainText("1.25 days");
  const source = panel(page, "Source conversion");
  const sourceTooltip = await hoverCategory(page, source, "Referral");
  await expect(sourceTooltip).toContainText("Conversion rate");
  await expect(sourceTooltip).toContainText("50%");
  await source.getByRole("button", { name: "Converted", exact: true }).click();
  await hoverCategory(page, source, "Referral");
  await expect(sourceTooltip).toContainText("Converted");
  await expect(sourceTooltip).toContainText("1,23,456");
  const stages = panel(page, "Canonical stage order");
  await expect(await hoverCategory(page, stages, "New")).toContainText("Leads");
