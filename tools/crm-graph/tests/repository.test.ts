...
test("transient untracked cleanup restarts the fingerprint snapshot instead of crashing",()=>{const root=repo();const transient=path.join(root,"transient.txt");fs.writeFileSync(transient,"temporary");let removed=false;const result=worktreeFingerprint(root,{beforeHashUntracked(relative){if(relative==="transient.txt"&&!removed){removed=true;fs.unlinkSync(transient);}}});assert.equal(removed,true);assert.deepEqual(result.untracked,[]);assert.equal(result.hash,worktreeFingerprint(root).hash);});
