...
-- Disable legacy automatic stage writers; all future stage changes use the audited authority.
do $$
begin
  if exists (select 1 from pg_extension where extname = 'pg_cron') then
    execute $cron$select cron.unschedule(jobname) from cron.job where jobname in ('nightly-renewals','nightly-reengage')$cron$;
  end if;
end $$;
