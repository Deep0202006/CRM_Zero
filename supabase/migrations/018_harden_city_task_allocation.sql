CREATE INDEX IF NOT EXISTS idx_alloc_targets_compound
  ON public.allocated_targets (assigned_to_user_id, is_completed, city);

CREATE OR REPLACE FUNCTION public.allocate_city_task_batch(
  p_filename text,
  p_file_hash text,
  p_rows jsonb,
  p_city_assignments jsonb
) RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  IF EXISTS (SELECT 1 FROM jsonb_each_text(p_city_assignments) m WHERE m.value !~ '^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$') THEN RAISE EXCEPTION 'invalid selected user' USING ERRCODE = '22023'; END IF;
  ...
  IF EXISTS (SELECT 1 FROM jsonb_each_text(p_city_assignments) m WHERE m.key <> lower(regexp_replace(btrim(m.key),'\s+',' ','g'))) THEN RAISE EXCEPTION 'invalid city mapping key' USING ERRCODE='22023'; END IF;
  ...
  INSERT INTO public.task_upload_batches(uploaded_by,filename,file_hash) VALUES(caller_id,p_filename,p_file_hash) RETURNING id INTO batch_id;
